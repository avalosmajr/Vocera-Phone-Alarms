...
## Results
  The most committed type of event were missed turns (60) in comparison to repeated turns (40), as seen in **Figure @fig-fig1**. The day and night shift was investigated, which showed consistency amongst both shifts with missed turn being the most common; however, the day shift was associated with 16 more total events than the night shift (**Figure @fig-fig1**)  furthermore, it was observed that the day 
...
```{r fig1, fig.width=8, fig.height=6, fig.cap="Amount of Events Based on Nursing Shift"}
ggplot(fake_nurse_shift.csv, aes(x= `Shift_Type`, fill= `Type of Event`)) +
