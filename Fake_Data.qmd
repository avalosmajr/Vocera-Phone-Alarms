...
```{r}
# Analyze the data for Patient(s)meanstdminmax
fake_data %>%
  count(`Patient ID`, sort = TRUE) %>%
...
fake_nurse_event_data <- fake_data 
...
fake_nurse_event_data_Perc <- fake_nurse_event_data %>%
 ...
 # Helping to move the data from long to wide data format
  pivot_wider(
...
## 2. Which nurses committed which event for instance Nurse C was associated with 9 events; therefore, how many were missed vs repeated turns
# Helping to move the data from long to wide data format
fake_nurse_event_data <- fake_nurse_event_data %>%
  group_by(`Nurse`, `Type of Event`) %>%
  summarize(Count = n(), .groups = "drop") %>%
  pivot_wider(names_from = `Type of Event`, values_from = Count, values_fill = list(Count = 0))

## 3.2. Fixing the deci placement 
...
mutate(Repeated_Turn_YesorNo = if_else(str_detect(`Type of Event`, "Repeated Turn"), "Yes", "No")) %>%
  mutate(Repeated_Turn_YesorNo = str_to_upper(Repeated_Turn_YesorNo))
...
fake_data <- fake_data %>%
  mutate(Nurse = str_to_title(Nurse),
        `Nurse Associated With Event` = str_to_title(`Nurse Associated With Event`))
...
ggplot(fake_nurse_event_data_Perc_long, aes(x = Nurse, y= Count, fill = `Event Type`)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8), width = 0.7) +
  labs(title = "Number of Events Per Nurse", x = "Nurse", y = "Event Count") +
  scale_fill_manual(values = c("Count Missed Turn" = "darkblue", "Count Repeated Turn" = "darkorange")) +
  theme_minimal() +
    theme(
...
fake_nurse_event_time_date <- fake_nurse_event_time_date  %>%
  mutate(`Week of Event` = as.Date(cut(`Week of Event`, breaks = "week"))) %>%
...
ggplot(fake_nurse_event_time_date, aes(x = `Week of Event`, y= `Count`, color = `Nurse`, group = interaction(`Nurse`, `Type of Event`))) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  geom_smooth(aes(group = `Type of Event`), method = "loess", se = FALSE, linetype = "dashed", color = "black", size = 1.2) +
...
mutate(`Nurse` = as.factor(`Nurse`),
    Hour = as.numeric(format(strptime(`Time of Event`, format = "%H:%M:%S"), "%H")), 
    Shift_Type = case_when(
      Hour >= 6 &
      Hour < 18 ~ "Day", 
TRUE ~ "Night")) %>%
mutate(
  Shift_Type = fct_recode(Shift_Type, "Day Shift" = "Day", "Night Shift" = "Night"),
   Shift_Type = fct_relevel(Shift_Type,"Day Shift", "Night Shift"),
  `Type of Event` = factor(`Type of Event`, levels = c("Missed Turn", "Repeated Turn")),
  `Nurse` = fct_lump(`Nurse`, n = 4)) %>%
   select(-Hour)
...
fake_patient_event <- fake_patient_event %>%
  mutate(Age_Group = cut(Age, breaks = c(20, 40, 60, 80, 100),
...
fake_data <- read_csv("/Users/michaelavalosjr./Repos for Rstudio/Please/fake_data.csv")
